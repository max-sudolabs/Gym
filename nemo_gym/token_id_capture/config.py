...
from nemo_gym.token_id_capture.protocols import installed_token_sink
...
class TokenIdCaptureConfig(BaseModel):
    ...
    def _validate(self) -> "TokenIdCaptureConfig":
        ...
        if directory is None:
            # A process that installed its own sink writes through that transport and never
            # constructs the file store, so it has no directory to give. Only the file store
            # needs a path.
            if installed_token_sink() is not None:
                return self
            raise ValueError(
                "token_id_capture_dir (or model_call_capture_dir) is required when "
                "token_id_capture_enabled=true and no sink has been installed"
            )
