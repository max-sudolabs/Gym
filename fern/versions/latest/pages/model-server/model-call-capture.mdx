...
Aggregate token metrics sum values reported by individual calls. If a call omits a token field, that aggregate is partial;
use the per-call value to distinguish unavailable evidence from a measured zero.
...
<Warning>
The rollout attachment is not self-contained because it omits request and response payloads. Use `CaptureStore` for the
full model-server record. Rollout collection includes these payloads when it builds
[`ng_trajectory`](/about/concepts/trajectory-format). Producers that report `multimodal_history_redacted` remove
`input_image` blocks from the projected trajectory, not from `CaptureStore`. A missing token detail is not a measured zero.
</Warning>

## Agent observations

Supported Agent Servers may also attach `ng_agent_observations`; producer behavior is independent of model-call capture.
It contains an unordered `records` list of typed agent invocations, semantic turns, tool-call intervals, explicit
